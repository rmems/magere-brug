...
impl Recipe {
    ...
    /// Existing GGUF files are registered as-is. `gguf_export` is only for
    /// converting a safetensors / HF / local-dir source, so a GGUF source on
    /// that type is a configuration error rather than a no-op conversion.
    ...
    fn validate_outputs(&self) -> Result<(), String> {
        ...
        if let Some(generated_format) = outputs.generated_format.as_deref() {
            ...
            match self.recipe_type {
                ...
                RecipeType::GgufExport if generated_format != "gguf" => {
                    return Err(format!(
        ...
        } else if self.recipe_type.is_pack() || self.recipe_type == RecipeType::GgufExport {
            return Err(format!(
    ...
    fn apply_register(&self, registry_override: Option<&Path>) -> Result<String, String> {
        ...
        out.push_str(&format!("  Registered to: {}\n", registry_path.display()));
        out.push_str(&format!(
            "  Emitted manifest: {}\n",
    ...
    /// Copy the source manifest and write a combine-for-AI handoff file next to
    /// the registry so registered artifacts can be benchmarked without conversion.
    fn emit_registered_artifacts(
        ...
        std::fs::create_dir_all(&handoff_dir).map_err(|e| {
            format!(
        ...
        let mut source_artifact = serde_json::Map::new();
        source_artifact.insert(
            "format".to_string(),
            Value::String(manifest.source_artifact.format.clone()),
        );
        source_artifact.insert(
            "path".to_string(),
            Value::String(manifest.source_artifact.path.clone()),
        );
        if let Some(checksum) = checksum {
            source_artifact.insert("checksum".to_string(), Value::String(checksum));
        }

        let mut benchmark_linkage = serde_json::Map::new();
        benchmark_linkage.insert("status".to_string(), Value::String("ready".to_string()));
        if let Some(pipeline_id) = pipeline_id {
            benchmark_linkage.insert("pipeline_id".to_string(), Value::String(pipeline_id));
        }

        let mut handoff = serde_json::Map::new();
        handoff.insert(
            "schema".to_string(),
            Value::String("magere-brug/combine-for-ai-handoff/1".to_string()),
        );
        handoff.insert(
            "recipe_id".to_string(),
            Value::String(self.recipe_id.clone()),
        );
        handoff.insert(
            "manifest_ref".to_string(),
            Value::String(manifest.metadata.manifest_id.clone()),
        );
        handoff.insert(
            "model_slug".to_string(),
            Value::String(manifest.model.slug.clone()),
        );
        handoff.insert(
        ...
        handoff.insert(
            "emitted_manifest".to_string(),
            Value::String(emitted_manifest.display().to_string()),
        );
        handoff.insert(
            "benchmark_linkage".to_string(),
            Value::Object(benchmark_linkage),
        );
        if let Some(kernel_types) = kernel_types {
            handoff.insert(
                "kernel_placeholders".to_string(),
        ...
        let serialized = serde_json::to_string_pretty(&Value::Object(handoff))
