...
pub fn decide_precision(tensor_name: &str, manifest: &crate::manifest::DissectManifest) -> TensorPrecision {
    // Preserve list wins
    if manifest.preserve.iter().any(|p| glob_match(&p.name, tensor_name)) {
        return TensorPrecision::Preserve;
    }
    // FP16 list
    if manifest.fp16.iter().any(|f| glob_match(&f.name, tensor_name)) {
